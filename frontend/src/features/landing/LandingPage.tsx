    ...
    <div className="min-h-screen bg-[var(--prussian-blue)] text-white overflow-x-hidden">
      <nav className="fixed top-0 w-full z-50 bg-[var(--prussian-blue)]/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 py-1 md:py-0 md:h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-[9px] md:text-sm md:gap-3">
            <div className="relative">
              <button onClick={() => setLangOpen(!langOpen)} className="flex items-center gap-0.5 md:gap-1 text-[var(--muted-blue)] hover:text-white md:px-2 md:py-1 md:rounded-lg md:hover:bg-white/5">
                ...
                <span className="md:hidden">{LANGS.find(l => l.code === (i18n.language?.split('-')[0] || 'en'))?.label}</span>
            ...
            <a href="/login" className="text-[var(--muted-blue)] hover:text-white transition-colors">{t('landing.nav.login')}</a>
            <a href="/register" className="text-[var(--orange)] font-medium hover:text-amber-400 transition-colors md:bg-[var(--orange)] md:text-black md:px-4 md:py-2 md:rounded-full md:hover:scale-105">{t('landing.nav.start')}</a>
          </div>
        </div>
      </nav>
      ...
      <footer className="py-4 md:py-12 px-6 border-t border-white/5">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-2 md:gap-4">
          ...
          <p className="text-[4px] md:text-sm text-[var(--muted-blue)]">{t('landing.footer')}</p>
        </div>
      </footer>
    </div>
  )
