...
describe('AppShell — mutex navigation (ADMIN-005)', () => {
  it('shows INSTALLER nav items when user is an installer', () => {
    useAuthStore.setState({
      user: { id: 'u1', email: 'inst@test.com', role: 'installer', tenantId: 't1' },
      accessToken: 'tok',
  ...
  it('shows ADMIN nav items when user is super_admin', () => {
    useAuthStore.setState({
      user: { id: 'u2', email: 'sa@test.com', role: 'super_admin', tenantId: 'sa-t1' },
      accessToken: 'tok',
      isAuthenticated: true,
      isSuperAdmin: true,
    })
  ...
  it('does NOT show installer nav items when user is super_admin', () => {
    useAuthStore.setState({
      user: { id: 'u3', email: 'sa2@test.com', role: 'super_admin', tenantId: 'sa-t2' },
      accessToken: 'tok',
      isAuthenticated: true,
      isSuperAdmin: true,
    })
  ...
  it('does NOT show admin nav items when user is an installer', () => {
    useAuthStore.setState({
      user: { id: 'u4', email: 'inst2@test.com', role: 'installer', tenantId: 't2' },
      accessToken: 'tok',
      isAuthenticated: true,
      isSuperAdmin: false,
    })
  ...
  it('removes old Tenants link in favor of admin nav items', () => {
    useAuthStore.setState({
      user: { id: 'u5', email: 'sa3@test.com', role: 'super_admin', tenantId: 'sa-t3' },
      accessToken: 'tok',
      isAuthenticated: true,
      isSuperAdmin: true,
    })
