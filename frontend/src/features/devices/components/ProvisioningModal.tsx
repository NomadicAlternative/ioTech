...
export function ProvisioningModal({
	...
	const [credentials, setCredentials] =
		useState<ProvisioningCredentials | null>(null);
	...
	async function sendViaSerial(creds: ProvisioningCredentials) {
		...
		while (Date.now() < deadline) {
			try {
				...
				const isDeviceLost =
					msg.includes("device has been lost") || msg.includes("disconnected");
				if (isDeviceLost) {
								...
								<div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 flex items-start gap-3">
									...
									<div>
										<p className="text-sm font-medium text-green-800">
											{t("devices.provisioning.browserOk")}
										</p>
										<p className="text-xs text-green-700 mt-0.5">
											{t("devices.provisioning.browserOkDesc")}
										</p>
									</div>
								</div>
								...
								<div className="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 flex items-start gap-3">
									...
									<div>
										<p className="text-sm font-medium text-amber-800">
											{t("devices.provisioning.browserFail")}
										</p>
										<p className="text-xs text-amber-700 mt-0.5">
											{t("devices.provisioning.browserFailDesc")}
										</p>
									</div>
								</div>
								<p className="text-sm text-muted-foreground">
									{t("devices.provisioning.browserFailHint")}
								</p>
						...
						<div className="space-y-3">
							<div className="space-y-1">
								<Label htmlFor="wifi-ssid">
									{t("devices.provisioning.ssidLabel")}
								</Label>
								<Input
									id="wifi-ssid"
									value={wifiSsid}
							...
							<div className="space-y-1">
								<Label htmlFor="wifi-password">
									{t("devices.provisioning.passwordLabel")}
								</Label>
								<Input
									id="wifi-password"
									type="password"
					...
					<div className="py-6 flex flex-col items-center gap-4">
						...
						<div className="text-center space-y-3">
							<p className="text-sm font-medium">
								{step === "connecting"
									? t("devices.provisioning.connecting")
									: t("devices.provisioning.sending")}
							</p>
							{step === "sending" && (
								<p className="text-sm text-muted-foreground">
				...
				{step === "done" && (
					<div className="space-y-4 py-2">
						<div className="rounded-lg bg-green-50 border border-green-200 px-4 py-3 flex items-start gap-3">
							...
							<div>
								<p className="text-sm font-medium text-green-800">
									{t("devices.provisioning.doneTitle")}
								</p>
								<p className="text-xs text-green-700 mt-0.5">
									{t("devices.provisioning.doneDesc")}
								</p>
							</div>
						</div>
						<Button
						...
						<div className="rounded-lg bg-destructive/10 border border-destructive/20 px-4 py-3 flex items-start gap-3">
							...
							<div>
								<p className="text-sm font-medium text-destructive">
									{t("devices.provisioning.errorTitle")}
								</p>
						...
						{credentials && (
							<div className="space-y-2">
								...
								{[
									...
									{
										label: "Backend URL",
										value: credentials.backend_url,
										key: "backend",
									},
									{
										label: "MQTT URL",
										value: credentials.mqtt_url,
										key: "mqtt",
									},
									{
										label: "Device Token",
										value: credentials.device_token,
										key: "token",
									},
								].map(({ label, value, key }) => (
