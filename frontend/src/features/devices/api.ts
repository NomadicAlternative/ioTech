...
export async function listDevices(
	...
	const res = await api.get<{ data: Device[]; meta: PaginationMeta }>(
		"/api/devices",
		{
			params: {
				page,
...
export async function getProvisioningCredentials(
	deviceId: string,
): Promise<ProvisioningCredentials> {
	const res = await api.get<{ data: ProvisioningCredentials }>(
		`/api/devices/${deviceId}/provisioning-credentials`,
	);
	return res.data.data;
}
...
export {
	listTemplatesFlat as listTemplates,
	getTemplate as fetchDeviceTemplate,
