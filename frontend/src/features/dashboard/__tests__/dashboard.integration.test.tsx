...
describe("DashboardListPage", () => {
	...
	it("creates dashboard and navigates to editor", async () => {
		vi.mocked(dashboardApi.fetchDashboards).mockResolvedValueOnce([]);
		vi.mocked(dashboardApi.createDashboard).mockResolvedValueOnce({
			...MOCK_DASHBOARD,
			id: "dash-new",
		});
		...
		await waitFor(() => {
			expect(
				container.querySelector('[data-testid="editor-page"]'),
			).toBeInTheDocument();
		});
	});
});
