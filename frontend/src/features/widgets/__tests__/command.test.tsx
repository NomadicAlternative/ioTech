...
describe('ToggleSwitchWidget', () => {
  ...
  it('renders ON state when telemetry value matches onValue', () => {
    act(() => {
      useTelemetryStore.getState().setTelemetry('device-1', 'relay', '1', Date.now())
    })
  ...
  it('sends POST command on toggle and shows optimistic state immediately', async () => {
    ...
    await waitFor(() => {
      expect(dashboardApi.sendDeviceCommand).toHaveBeenCalledWith('device-1', 'on')
    })
  })

  it('reverts to original state on API error', async () => {
    vi.mocked(dashboardApi.sendDeviceCommand).mockRejectedValueOnce(new Error('API error'))
...
describe('ButtonWidget', () => {
  ...
  it('renders with label from config settings', () => {
    render(
      <ButtonWidget
        widgetId="w2"
        config={makeConfig({ settings: { action: 'trigger', label: 'Activate', variant: 'default' } })}
        isEditing={false}
      />
    )
  ...
  it('sends command with configured action on click', async () => {
    ...
    render(
      <ButtonWidget
        widgetId="w2"
        config={makeConfig({ settings: { action: 'open', label: 'Open Gate', variant: 'default' } })}
        isEditing={false}
      />
    )
  ...
  it('disables button while command is in flight', async () => {
    ...
    render(
      <ButtonWidget
        widgetId="w2"
        config={makeConfig({ settings: { action: 'trigger', label: 'Send', variant: 'default' } })}
        isEditing={false}
      />
    )
  ...
  it('does not send command when deviceId is empty', async () => {
    render(
      <ButtonWidget
        widgetId="w2"
        config={makeConfig({ deviceId: '', settings: { action: 'trigger', label: 'Send', variant: 'default' } })}
        isEditing={false}
      />
    )
